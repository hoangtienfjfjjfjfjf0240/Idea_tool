...
import { MessageCircle, Send, X, Bot, User, Loader2, Sparkles, ChevronDown, Lightbulb, TrendingUp, Target, Zap } from 'lucide-react';
import type { AppProject, Hook, GeneratedIdea } from '@/types/database';
...
export const ChatAgent: React.FC<ChatAgentProps> = ({ app, appContext, onIdeasGenerated }) => {
  ...
  const sendMessage = async (text?: string) => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        // If ideas were generated, notify parent
        if (data.ideas && onIdeasGenerated) {
  ...
  const formatContent = (content: string) => {
    // Remove JSON blocks (they'll be rendered as idea cards)
              ...
              {msg.ideas && msg.ideas.length > 0 && (
                <div style={{ marginTop: 12, padding: '10px 12px', background: '#f0fdf4', borderRadius: 10, border: '1px solid #bbf7d0' }}>
                  <div style={{ fontSize: 11, fontWeight: 600, color: '#16a34a', marginBottom: 6 }}>
                    ✨ {msg.ideas.length} Ideas đã tạo
                  </div>
                  {msg.ideas.map((idea: GeneratedIdea, j: number) => (
